...
class Stalk extends React.PureComponent<any, any> {
  ...
  render() {
    ...
    const follower = !this.props.loadingFollwer ? (
      this.props.follower ? (
        this.props.follower.map((el: any) => {
          return (
            <a href={el.html_url} key={el.id}>
              <Tooltip title="Click to checkout on GitHub">
                <Card.Grid
                  style={{
                    width: "30%",
                    overflow: "hidden",
    ...
    const following = !this.props.loadingFollwing ? (
    ...
    const organization = !this.props.loadingOrg ? (
      this.props.organization ? (
        this.props.organization.map((el: any) => {
          return (
            <a href={`https://github.com/${el.login}`} key={el.id}>
              <Tooltip title="Click to checkout on GitHub">
                <Card.Grid
                  style={{
                    width: "30%",
      ...
      ? this.props.events.map((el: any) => {
          ...
          if (el.type === "IssueCommentEvent") {
            event = (
              ...
              >
                <Icon type="edit" theme="twoTone" /> Commented on the Issue in
                the Repository:{" "}
            ...
            event = (
              ...
              >
                {" "}
                Starred
                <Icon type="star" theme="twoTone" twoToneColor="#ffd761" /> the
                Repository:{" "}
                <a href={`https://github.com/${el.repo.name}`}>
                  {el.repo.name}
                </a>
              </Card.Grid>
            );
            ...
            el.type === "IssuesEvent" &&
            (el.payload.action === "opened" || el.payload.action === "closed")
          ) {
            event = (
              <Card.Grid
                key={el.id}
              ...
              >
                ...
                {el.payload.action === "opened" ? (
                  <>
                    <Icon type="folder-open" theme="twoTone" /> Opened an Issue
                    in the Repository:{" "}
                  </>
                ) : (
                  <>
            ...
            event = (
              ...
              >
                ...
                {el.payload.action === "opened" ? (
                  <>
                    <Icon type="folder-open" theme="twoTone" /> Opened a Pull
            ...
            event = (
              <Card.Grid
                ...
                style={{ overflow: "hidden", width: "100%" }}
              >
                <Icon type="edit" theme="twoTone" /> Pushed a commit in the
                Repository:{" "}
                <a href={`https://github.com/${el.repo.name}`}>
                  {el.repo.name}
                </a>
            ...
            event = (
              <Card.Grid
                ...
                style={{ overflow: "hidden", width: "100%" }}
              >
                <Icon type="edit" theme="twoTone" /> Commented on the Pull
                Request in the Repository:{" "}
                <a href={`${el.payload.pull_request.html_url}`}>
                  {el.repo.name}
                </a>
          ...
          return el.type === "PullRequestEvent" ||
            "WatchEvent" ||
            "IssuesEvent" ||
            "IssueCommentEvent" ||
            "PushEvent" ||
        ...
        <>
          <div>
            <Input.Search
              ...
              onSearch={async (user) => await this.props.setUserName(user)}
            />
          </div>
          <div>
            <Empty />
    ...
    return (
      <>
        ...
        <div className={classes.align}>
          {data ? (
            <Tooltip placement="right" title="Checkout on GitHub">
              <Card
                hoverable
                style={{ width: 300 }}
                cover={<img src={data.avatar_url} alt="avatar" />}
              ...
              >
                ...
                {data.hireable ? (
                  <div>
                    Hireable:{" "}
                    {data.hireable ? <>Of Course!</> : <>Sad but No!</>}
                  </div>
                ) : null}
...
const mapStateToProps = ({ gitrepoReducer }: { gitrepoReducer: any }) => ({
  ...
  organization: gitrepoReducer.organization
    ? gitrepoReducer.organization.data
    : [],
